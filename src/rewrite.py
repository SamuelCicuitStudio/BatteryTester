...
def replace_serial_prints(file_path):
    ...
    # === Step 1: Replace Serial.println() and Serial.print() ===
    content = re.sub(r'\bSerial\.println\s*\(', 'DEBUG_PRINTLN(', content)
    content = re.sub(r'\bSerial\.print\s*\(', 'DEBUG_PRINT(', content)

    # === Step 2: Remove "if (DEBUGMODE)" from single-line debug statements ===
    # Transforms: if (DEBUGMODE) DEBUG_PRINTLN("...");
    content = re.sub(
        r'\bif\s*\(\s*DEBUGMODE\s*\)\s*(DEBUG_PRINTLN|DEBUG_PRINT)\s*\(',
        r'\1(',
        content
    )
